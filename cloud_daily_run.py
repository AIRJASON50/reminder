...
# ===========================================================================
# 2. 数据拉取 — BaoStock 批量拉取主板股票 120 天日线
# ===========================================================================
def fetch_all_stock_data(days: int = 150) -> tuple[dict, dict]:
    ...
    # 获取股票列表
    log.info("获取股票列表...")
    rs_list = bs.query_stock_industry()
    stock_rows = []
    while rs_list.error_code == "0" and rs_list.next():
        stock_rows.append(rs_list.get_row_data())
    industry_df = pd.DataFrame(stock_rows, columns=rs_list.fields)

    # 另外用 query_stock_basic 获取名称
    ...
    basic_df = basic_df[basic_df["status"] == "1"]  # 1=上市
    basic_df["pure_code"] = basic_df["code"].str.split(".").str[1]
    basic_df = basic_df[
        basic_df["pure_code"].str.match(r"^(00|60)\d{4}$")
    ]
